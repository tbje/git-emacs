...
(let ((map (make-keymap)))
  ...
  (define-key map "\C-u" 'git--status-view-unstage)
  ...
  (let* ((files (git--status-view-marked-or-file))
    ...
    (let* ((untracked-warn (git--bold-face "untracked"))
      ...
      (let ((tracked-files
            ...
            (num-deleted 0))
        (when tracked-files
          (apply #'git--exec-string "reset" "HEAD" tracked-files))
        (incf num-deleted (length tracked-files))
        ;; Remove other files directly
        (unwind-protect
          (message "Resat out %d files" num-deleted)))))
